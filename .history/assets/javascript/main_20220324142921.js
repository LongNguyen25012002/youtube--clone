...
function sliderCarouselhandle(){
    ...
    var itemCategory = [...document.querySelectorAll('.category-item')];
        itemCategory.forEach(function(element){
            element.getBoundingClientRect().width;
        })
    var widthToMove = 150;
    var arrowBtnWidth = 50;
    var currentSlider  = 1;
    var carouseLeftValue = 0;
    var carouseRightValue = 0;

         carouseLeftBTN.addEventListener('click',moveCarouseLeft);
         carouseRightBTN.addEventListener('click',moveCarouseRight);

         function moveCarouseLeft(){
             currentSlider++;
             if(currentSlider == 1){
                 carouseLeftValue = 0;
             } else {
                 carouseLeftValue +=  widthToMove;
             }

             if(carouseLeftValue > totalWidth){
                carouselContent.style.right = 0;
             }
            
             console.log(carouseLeftValue);
             carouselContent.style.left =  `-${carouseLeftValue}px` ;

         }

         function moveCarouseRight(){
            currentSlider--;
            console.log(currentSlider);
            if(currentSlider == 2){
                carouseRightValue = 0;
            } else {
                carouseRightValue +=  widthToMove;
            }
           
            console.log(carouseRightValue);
            carouselContent.style.right =  `-${carouseRightValue}px` ;
        }
}
